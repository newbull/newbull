...
#if defined(NDEBUG)
#error "NewBull cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "NewBull Signed Message:\n";
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        BOOST_FOREACH (const CTxMemPool::txiter it, allConflicting) {
            LogPrint("mempool", "replacing tx %s with %s for %s NB additional fees, %d delta bytes\n",
...
public:
    ...
    {
        // if (pindex->nVersion != pindex->pprev->nVersion) {
        //     printf("main Condition, h: %d, ", pindex->nHeight);
        //     printf("ver=%x, TOP_MASK=%x, TOP_BITS=%x, bit=%d, pprev_ver=%x\n", pindex->nVersion, VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS, bit, ComputeBlockVersion(pindex->pprev, params));
        //     bool a = ((pindex->nVersion & VERSIONBITS_TOP_MASK) == VERSIONBITS_TOP_BITS);
        //     bool b = (((pindex->nVersion >> bit) & 1) != 0);
        //     bool c = (((ComputeBlockVersion(pindex->pprev, params) >> bit) & 1) == 0);
        //     if (a && b && c) {
        //         printf("true, true, true\n");
        //     } else {
        //         printf("%s, %s, %s\n", a ? "true" : "false", b ? "true" : "false", c ? "true" : "false");
        //     }
        //     // printf("(pindex->nVersion & VERSIONBITS_TOP_MASK)=%x, (%x == %x)=%s\n", (pindex->nVersion & VERSIONBITS_TOP_MASK), (pindex->nVersion & VERSIONBITS_TOP_MASK), VERSIONBITS_TOP_BITS, ((pindex->nVersion & VERSIONBITS_TOP_MASK) == VERSIONBITS_TOP_BITS) ? "true" : "false");
        //     // printf("(pindex->nVersion >> %d)=%x, (%x(%d) & 1)=%x, %s\n", bit, (pindex->nVersion >> bit), (pindex->nVersion >> bit), (pindex->nVersion >> bit), ((pindex->nVersion >> bit) & 1), ((pindex->nVersion >> bit) & 1) != 0 ? "ture" : "false");
        //     // printf("(ComputeBlockVersion(pindex->pprev, params) >> bit)=%x, ((%x >> %d) & 1)=%x, %s\n", (ComputeBlockVersion(pindex->pprev, params) >> bit), ComputeBlockVersion(pindex->pprev, params), bit, ((ComputeBlockVersion(pindex->pprev, params) >> bit) & 1), ((ComputeBlockVersion(pindex->pprev, params) >> bit) & 1) == 0 ? "true" : "false");
        //     // printf("(20000002 & %x)=%x, %s\n", VERSIONBITS_TOP_MASK, (20000002 & VERSIONBITS_TOP_MASK), ((20000002 & VERSIONBITS_TOP_MASK) == VERSIONBITS_TOP_BITS) ? "true" : "false");
        //     // printf("(20000002 >> %d)=%x, (%x(%d) & 1)=%x, %s\n", bit, (20000002 >> bit), (20000002 >> bit), (20000002 >> bit), ((20000002 >> bit) & 1), ((20000002 >> bit) & 1) != 0 ? "ture" : "false");
        //     // printf("(ComputeBlockVersion(pindex->pprev, params) >> bit)=%x, ((%x >> %d) & 1)=%x, %s\n", (20000002 >> bit), 20000002, bit, ((20000002 >> bit) & 1), ((20000002 >> bit) & 1) == 0 ? "true" : "false");
        // }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // case 263379:
    // case 263380:
    //     printf("hashMerkleRoot mismatch: h=%d, %x\n", pindex->nHeight, block.hashMerkleRoot);
    //     // printf("hashMerkleRoot mismatch: h=%d, %x\n", pindex->nHeight, block.hashMerkleRoot);
    //     break;
    // default:
    //     // Check it again in case a previous version let a bad block in
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        // switch (chainActive.Height() + 1) {
        // case 263380:
        //     // printf("hashMerkleRoot mismatch: h=%s, %s!=%s\n", block.ToString(), hashMerkleRoot, hashMerkleRoot2);
        //     printf("hashMerkleRoot mismatch: h=%d, %x!=%x\n", chainActive.Height() + 1, block.hashMerkleRoot, hashMerkleRoot2);
        //     break;
        // default:
        if (block.hashMerkleRoot != hashMerkleRoot2) {
            // return state.DoS(100, false, REJECT_INVALID, "bad-txnmrklroot", true, "hashMerkleRoot mismatch");
            // std::ostringstream oss;
            // oss << "hashMerkleRoot mismatch: " << block.hashMerkleRoot << "!=" << hashMerkleRoot2;
            char* buf = new char[strlen("hashMerkleRoot mismatch: ") + 128 + 3];
            sprintf(buf, "hashMerkleRoot mismatch: %x!=%x", block.hashMerkleRoot, hashMerkleRoot2);
            // printf("hashMerkleRoot mismatch: h=%s, %s!=%s\n", chainActive.Height(), block.hashMerkleRoot, hashMerkleRoot2);
            printf("hashMerkleRoot mismatch: h=%d, %x!=%x\n", chainActive.Height() + 1, block.hashMerkleRoot, hashMerkleRoot2);
            return state.DoS(100, false, REJECT_INVALID, "bad-txnmrklroot", true, buf);
        }
